...
#start worker nodes
SPARK_NO_DAEMONIZE=1 srun -n 790 -N ${NWORKERS} -r 1 --label --output=$SPARK_LOG_DIR/spark-%j-workers.out start-slave.sh -m 8g -c ${SLURM_CPUS_PER_TASK} ${MASTER_URL} & slaves_pid=$!

srun -n 1 -N 1 spark-submit --master ${MASTER_URL} --driver-memory 80g --conf spark.driver.memoryOvehead=8g --executor-memory 6g --conf spark.executor.memoryOverhead=0.6g --conf spark.memory.fraction=0.1 --conf spark.driver.maxResultSize=0 --conf spark.memory.storageFraction=0.5 --conf spark.default.parallelism=18960 --conf spark.sql.shuffle.partitions=18960 --conf spark.shuffle.io.maxRetries=10 --conf spark.blacklist.enabled=False  --conf spark.shuffle.io.retryWait=60s --conf spark.reducer.maxReqsInFlight=1 --conf spark.shuffle.io.backLog=4096 --conf spark.network.timeout=1200 --conf "spark.executor.extraJavaOptions=-XX:+PrintGCDetails -XX:+PrintGCTimeStamps -Xloggc:$SCRATCH/log/ -XX:+UseCompressedOops" --conf spark.serializer=org.apache.spark.serializer.KryoSerializer --conf spark.scheduler.listenerbus.eventqueue.capacity=20000 --packages graphframes:graphframes:0.8.0-spark2.4-s_2.11  --repositories https://repos.spark-packages.org DistributedLayoutAlgorithm.py SNAP/com-friendster.ungraph.txt output/ 100
kill $slaves_pid
stop-master.sh
